...
namespace Level
{
	internal class SaveData
    {
		...
		public bool Open(String filename)
		{
			...
			buffer = Decomp(ref tmp);
			if (buffer.Length == 0) return false;

			buffer = Decomp(ref buffer);
			if (buffer.Length == 0) return false;
		...
		public bool Save()
		{
			...
			var buffer = Comp(ref mBuffer);
			var length = buffer.Length;
			buffer = Comp(ref buffer);
		...
		private Byte[] Decomp(ref Byte[] buffer)
		{
		...
		private Byte[] Comp(ref Byte[] buffer)
		{
			Byte[] result = new Byte[0];
			using (var input = new MemoryStream(buffer))
			{
